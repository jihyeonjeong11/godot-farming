...
- 최상위 플레이어 커서
- [] pause 상태가 아닐 시 여기서만 마우스 클릭/우클릭 받음
- [] 예외 - 애니메이션과는 별게 애니메이션은 무조건 재생되어야 하므로 클릭 라이트 클릭 받음

1.  최상위 farm의 아래에는 player_cursor component가 있음
	여기서 컨트롤 관련을 가져감 1과 같죠 2. 매니저는 삭제함 3. 커서 컴포넌트를 다이내믹하게 하나만 만들고 각 컴포넌트가
	레이어별로 관장할 수 있도록 함. 그러면 위 1.에서 타일 위치를 받아와서 선택하는 타일 위치를 받을 수 있겠죠 드롭된
	아이템은 그대로 좌표로 가져가면 되고, 4. 마지막으로 세이브한 것들은 글로벌로 빼고 farm의 의졵성을 뺀다 이러면 좀
	클린해질거같은데 어떰?

## 오브젝트들

- [] 오브젝트 tres 추가하고 tscn 하나로 통일
- [] 오브젝트 상태 머신 추가. 스프라이트 바뀌는 경우 ex)불 드럼통, 사과나무 등
- [x] ysort
